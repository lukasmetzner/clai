...
func execCommandInPod(podName string, command []string, stdinBuffer *bytes.Buffer) error {
...
func executeJob(job models.Job) {
	...
	if err := execCommandInPod(podName, command, &buffer); err != nil {
		log.Printf("%s\n", err)
	}
	...
	if err := execCommandInPod(podName, command, nil); err != nil {
		log.Printf("%s\n", err)
	}
	...
	if err := execCommandInPod(podName, command, &scriptBuffer); err != nil {
		log.Printf("%s\n", err)
	}
